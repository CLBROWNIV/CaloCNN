...
def train_model(model, dataloader, criterion, optimizer, num_epochs=10):
    ...
    for epoch in range(num_epochs):
        ...
        if running_loss / len(dataloader) < 10:
            for param_group in optimizer.param_groups:
                param_group['lr'] = .00001  # Decrease LR by a factor of 10
...
if __name__ == "__main__":
    ...
    plt.plot(range(1, len(loss_history) + 1), loss_history, color='k', linestyle='-')#, linestyle='-')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.title('Training Loss vs. Epoch')
    plt.yscale('log')
    plt.savefig("LOSS_LESS_THAN_10.png")
